...
pub fn sub_created(env: &Env, id: u64, owner: &Address, watched: &Address) {
    env.events().publish(
        (symbol_short!("sub_new"), owner.clone()),
        (id, watched.clone()),
    );
}
...
pub fn sub_cancelled(env: &Env, id: u64, owner: &Address) {
    env.events().publish(
        (symbol_short!("sub_cncl"), owner.clone()),
        id,
    );
}
...
pub fn sub_paused(env: &Env, id: u64, owner: &Address) {
    env.events().publish(
        (symbol_short!("sub_paus"), owner.clone()),
        id,
    );
}
...
pub fn sub_resumed(env: &Env, id: u64, owner: &Address) {
    env.events().publish(
        (symbol_short!("sub_rsm"), owner.clone()),
        id,
    );
}
...
pub fn sub_renewed(env: &Env, id: u64, owner: &Address, new_expires_at: u32) {
    env.events().publish(
        (symbol_short!("sub_renew"), owner.clone()),
        (id, new_expires_at),
    );
}
...
pub fn admin_transferred(env: &Env, old_admin: &Address, new_admin: &Address) {
    env.events().publish(
        (symbol_short!("adm_xfr"), old_admin.clone()),
        new_admin.clone(),
    );
}
...
pub fn onchain_sub_activated(env: &Env, id: u64, owner: &Address, watched: &Address) {
    env.events().publish(
        (symbol_short!("oc_live"), owner.clone()),
        (id, watched.clone()),
    );
}
